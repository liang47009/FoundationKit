#ifndef LOSEMYMIND_SOCKET_OPTION_HPP
#define LOSEMYMIND_SOCKET_OPTION_HPP
...
public:
    ...
    template <typename Protocol>
    network::linger_type* data(const Protocol&)
    {
        return &value_;
    }
    ...
    // Get the address of the linger data.
    template <typename Protocol>
    const network::linger_type* data(const Protocol&) const
    {
        return &value_;
    }
...
public:
    ...
    explicit multicast_request(const network::ip::address& multicast_address)
        : ipv4_value_(), // Zero-initialisation gives the "any" address.
        ipv6_value_() // Zero-initialisation gives the "any" address.
    {
        if (multicast_address.is_v6())
        {
            ...
            network::ip::address_v6 ipv6_address = multicast_address.to_v6();
            network::ip::address_v6::bytes_type bytes = ipv6_address.to_bytes();
        ...
        else
        {
            ipv4_value_.imr_multiaddr.s_addr =
                network::socket_ops::host_to_network_long(
                multicast_address.to_v4().to_ulong());
            ipv4_value_.imr_interface.s_addr =
                network::socket_ops::host_to_network_long(
                network::ip::address_v4::any().to_ulong());
        }
    }

    // Construct with multicast address and IPv4 address specifying an interface.
    explicit multicast_request(
        const network::ip::address_v4& multicast_address,
        const network::ip::address_v4& network_interface
        = network::ip::address_v4::any())
        : ipv6_value_() // Zero-initialisation gives the "any" address.
    {
        ipv4_value_.imr_multiaddr.s_addr =
            network::socket_ops::host_to_network_long(
    ...
    explicit multicast_request(
        const network::ip::address_v6& multicast_address,
        unsigned long network_interface = 0)
        : ipv4_value_() // Zero-initialisation gives the "any" address.
    {
        using namespace std; // For memcpy.
        network::ip::address_v6::bytes_type bytes =
...
private:
    network::in4_mreq_type ipv4_value_;
    network::in6_mreq_type ipv6_value_;
};
...
public:
    ...
    explicit network_interface(const network::ip::address_v4& ipv4_interface)
    {
        ipv4_value_.s_addr =
            network::socket_ops::host_to_network_long(
...
private:
    network::in4_addr_type ipv4_value_;
...
#endif // LOSEMYMIND_SOCKET_OPTION_HPP
