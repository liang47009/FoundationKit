...
LuaStack::Pointer LuaStack::create(void)
{
...
LuaStack::Pointer LuaStack::attach(lua_State *L)
{
    LuaStack *stack = new (std::nothrow) LuaStack();
...
void LuaStack::addSearchPath(const char* path)
{
    ...
    /* package.path = newpath, L: package path */
...
{
    luaevent_remove_function_by_refid(_state, nHandler);
}
...
int LuaStack::executeScriptFile(const char* filename)
{
    ...
    if (!data.empty())
    {
        if (luaLoadBuffer(_state, data.c_str(), (int)data.size(), fullPath.c_str()) == 0)
        {
...
int LuaStack::executeGlobalFunction(const char* functionName)
{
    ...
    {
        LOG_ERROR("[LUA ERROR] name '%s' does not represent a Lua function", functionName);
        lua_pop(_state, 1);
        return 0;
    }
    return executeFunction(0);
...
int LuaStack::executeGlobalFunction(const char* functionName, const LuaValueArray& valueArray)
{
    lua_getglobal(_state, functionName);       /* query function by name, stack: function */
    if (!lua_isfunction(_state, -1))
    {
    ...
    this->pushLuaValueArray(valueArray);
    return executeFunction(static_cast<int>(valueArray.size()));
}
...
void LuaStack::pushLuaValueArray(const LuaValueArray& valueArray)
{
    lua_newtable(_state);                                              /* L: table */
    int index = 1;
    for (LuaValueArrayIterator it = valueArray.begin(); it != valueArray.end(); ++it)
    {
...
{
    luaevent_get_function_by_refid(_state, nHandler);                  /* L: ... func */
    if (!lua_isfunction(_state, -1))
    {
        LOG_INFO("[LUA ERROR] function refid '%d' does not reference a Lua function", nHandler);
        lua_pop(_state, 1);
        return false;
    }
    return true;
...
int LuaStack::executeFunction(int numArgs)
{
    ...
    {
        LOG_INFO("value at stack [%d] is not function", functionIndex);
    ...
    if (error)
    {
        if (traceback == 0)
        {
            LOG_ERROR("[LUA ERROR] %s", lua_tostring(_state, -1));        /* L: ... error */
...
int LuaStack::reallocateScriptHandler(int nHandler)
{
    ...
    {
        nNewHandle = static_cast<int>(luaevent_ref_function(_state, lua_gettop(_state), 0));
    }
    /*
...
int LuaStack::executeFunction(int handler, int numArgs, int numResults, const std::function<void(lua_State*, int)>& func)
{
    if (pushFunctionByHandler(handler))                 /* L: ... arg1 arg2 ... func */
    {
        ...
        if (error)
        {
            if (traceCallback == 0)
            {
                LOG_INFO("[LUA ERROR] %s", lua_tostring(_state, -1));        /* L: ... error */
                lua_pop(_state, 1);                                        // remove error message from stack
            }
            else                                                           /* L: ... G error */
            {
...
int LuaStack::reload(const char* moduleFileName)
{
    ...
    {
        LOG_INFO("moudulFileName is null");
        return 1;
    }
...
int LuaStack::luaLoadBuffer(lua_State *L, const char *chunk, int chunkSize, const char *chunkName)
{
    ...
    if (r)
    {
        ...
        {
        case LUA_ERRSYNTAX:
            LOG_INFO("[LUA ERROR] load \"%s\", error: syntax error during pre-compilation.", chunkName);
            break;

        case LUA_ERRMEM:
            LOG_INFO("[LUA ERROR] load \"%s\", error: memory allocation error.", chunkName);
            break;

        case LUA_ERRFILE:
            LOG_INFO("[LUA ERROR] load \"%s\", error: cannot open/read file.", chunkName);
            break;
