...
public:
    ...
    void permutation();
    void keygen();
    int Encrypt(unsigned char* output, int outlen, unsigned char* input, int inlen);
    int Decrypt(unsigned char* output, int outlen, unsigned char* input, int inlen);
};
void DesImpl::IP()
{
    int k = 58, i;
...
void DesImpl::xor_oneE(int round)
{
    int i;
    for (i = 0; i < 48; i++)
...
void DesImpl::xor_oneD(int round)
{
    int i;
    for (i = 0; i < 48; i++)
...
int DesImpl::Encrypt(unsigned char* output, int outlen, unsigned char* input, int inlen)
{
    int resultLen = 0;
...
int DesImpl::Decrypt(unsigned char* output, int outlen, unsigned char* input, int inlen)
{
    int resultLen = 0;
...
int desEncrypt(unsigned char* output, int outlen, unsigned char* input, int inlen)
{
    ...
    while (inlen) 
    {
        ...
        char c = static_cast<char>(BLOCK_SIZE - j); // pad with the character that corresponds to how many bytes we need to pad
        unsigned int orig_block_size = j;
        for (; j < BLOCK_SIZE; j++) // if we didn't get 8 bytes, pad plaintext
            plaintext[j] = c;
        int encodeLen = desimpl.Encrypt(ciphertext, BLOCK_SIZE, plaintext, BLOCK_SIZE);
        if (encodeLen != BLOCK_SIZE && outlen < BLOCK_SIZE) // not enough room to write out
            return -1;
        memcpy(output, ciphertext, BLOCK_SIZE);
        output += BLOCK_SIZE;
        outlen -= BLOCK_SIZE;

        if (orig_block_size == BLOCK_SIZE && inlen == 0) { // the last chunk was 8 bytes.  encrypt another round so we know how to decrypt the padding
            memset(plaintext, BLOCK_SIZE, BLOCK_SIZE); // pad 8 bytes with the value 8
            encodeLen = desimpl.Encrypt(ciphertext, BLOCK_SIZE, plaintext, BLOCK_SIZE);
            if (encodeLen != BLOCK_SIZE && outlen < BLOCK_SIZE) // not enough room to write out
                return -1;
            memcpy(output, ciphertext, BLOCK_SIZE);
            output += BLOCK_SIZE;
...
int desDecrypt(unsigned char* output, int outlen, unsigned char* input, int inlen)
{
    ...
    while (inlen)
    {
        ...
        if (inlen < BLOCK_SIZE) // the ciphered text should be in 8 byte chunks
            return -1;
