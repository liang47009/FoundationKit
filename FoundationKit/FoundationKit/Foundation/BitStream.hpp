...
public:
    ...
    template <class _Ty>
    void Write(const _Ty &inTemplateVar);
    ...
    uint32_t CopyData(unsigned char** _data) const;
    ...
    void WriteBits(const unsigned char* inByteArray, uint32_t numberOfBitsToWrite, const bool rightAlignedBits = true);
    ...
    bool ReadBits(unsigned char *inOutByteArray, uint32_t numberOfBitsToRead, const bool alignBitsToRight = true);
    ...
    inline void Write(const char * const inStringVar)
    {
        ...
        this->Write(l);
        this->WriteAlignedBytes((const unsigned char*)inStringVar, (const unsigned int)l);
    }
...
inline void BitStream::Write(const _Ty &inTemplateVar)
{
    if (sizeof(inTemplateVar) == 1)
        WriteBits((unsigned char*)& inTemplateVar, sizeof(_Ty) * 8, true);
    else
    {
#ifndef __BITSTREAM_NATIVE_END
        if (DoEndianSwap())
        {
            unsigned char output[sizeof(_Ty)];
            ReverseBytes((unsigned char*)&inTemplateVar, output, sizeof(_Ty));
            WriteBits((unsigned char*)output, sizeof(_Ty) * 8, true);
        }
        else
#endif
            WriteBits((unsigned char*)& inTemplateVar, sizeof(_Ty) * 8, true);
...
inline void BitStream::Write(const std::string &inTemplateVar)
{
    ...
    this->Write(l);
    this->WriteAlignedBytes((const unsigned char*)inTemplateVar.c_str(), (const unsigned int)l);
}
template <>
...
{
    if (sizeof(inTemplateVar) == 1)
        WriteCompressed((unsigned char*)& inTemplateVar, sizeof(_Ty) * 8, true);
    else
    {
#ifndef __BITSTREAM_NATIVE_END
        if (DoEndianSwap())
        {
            unsigned char output[sizeof(_Ty)];
            ReverseBytes((unsigned char*)&inTemplateVar, output, sizeof(_Ty));
            WriteCompressed((unsigned char*)output, sizeof(_Ty) * 8, true);
        }
        else
#endif
            WriteCompressed((unsigned char*)& inTemplateVar, sizeof(_Ty) * 8, true);
...
inline bool BitStream::Read(_Ty &outTemplateVar)
{
    if (sizeof(outTemplateVar) == 1)
        return ReadBits((unsigned char*)&outTemplateVar, sizeof(_Ty) * 8, true);
    else
    {
#ifndef __BITSTREAM_NATIVE_END
        if (DoEndianSwap())
        {
            unsigned char output[sizeof(_Ty)];
            if (ReadBits((unsigned char*)output, sizeof(_Ty) * 8, true))
            {
                ReverseBytes(output, (unsigned char*)&outTemplateVar, sizeof(_Ty));
                return true;
            }
            return false;
        }
...
inline bool BitStream::Read(std::string &outTemplateVar)
{
    ...
    if (l > 0)
    {
        ...
        b = this->ReadAlignedBytes((unsigned char*)outTemplateVar.c_str(), l);
        if (!b)
            outTemplateVar.clear();
    }
    else
...
#endif
    if (sizeof(outTemplateVar) == 1)
        return ReadCompressed((unsigned char*)&outTemplateVar, sizeof(_Ty) * 8, true);
    else
    {
#ifndef __BITSTREAM_NATIVE_END
        if (DoEndianSwap())
        {
            unsigned char output[sizeof(_Ty)];
            if (ReadCompressed((unsigned char*)output, sizeof(_Ty) * 8, true))
            {
                ReverseBytes(output, (unsigned char*)&outTemplateVar, sizeof(_Ty));
                return true;
            }
...
void BitStream::WriteBitsFromIntegerRange(const _Ty value, const _Ty minimum, const _Ty maximum, const int requiredBits, bool allowOutsideRange)
{
    ...
    {
        unsigned char output[sizeof(_Ty)];
        ReverseBytes((unsigned char*)&valueOffMin, output, sizeof(_Ty));
