...
{
	ASSERTED( lengthInBits >= ( uint32_t ) numberOfBitsAllocated, _FILE_AND_LINE_ );
...
bool BitStream::Read( BitStream *bitStream, uint32_t numberOfBits )
{
...
//		return;
	...
	const uint32_t numberOfBitsUsedMod8 = numberOfBitsUsed & 7;
...
void BitStream::AddBitsAndReallocate( const uint32_t numberOfBitsToWrite )
{
...
{
	ASSERTED( readOffset == numberOfBitsUsed, _FILE_AND_LINE_ );
}
...
void BitStream::PrintBits( char *out ) const
{
	...
	for ( uint32_t counter = 0; counter < BITS_TO_BYTES( numberOfBitsUsed ) && strIndex < 2000 ; counter++ )
	{
		...
		for ( uint32_t counter2 = 7; counter2 >= stop; counter2-- )
		{
...
void BitStream::IgnoreBits( const uint32_t numberOfBits )
{
...
uint32_t BitStream::GetNumberOfBitsUsed( void ) const
{
...
void BitStream::SetReadOffset( const uint32_t newReadOffset )
{
...
bool BitStream::ReadFloat16( float &outFloat, float floatMin, float floatMax )
{
	...
	if (Read(percentile))
	{
        ASSERTED(floatMax>floatMin, _FILE_AND_LINE_);
...
void BitStream::WriteFloat16( float inOutFloat, float floatMin, float floatMax )
{
	...
	{
        ASSERTED(inOutFloat<=floatMax+.001, _FILE_AND_LINE_);
	}
	if (inOutFloat<floatMin-.001)
	{
        ASSERTED(inOutFloat>=floatMin-.001, _FILE_AND_LINE_);
	}
