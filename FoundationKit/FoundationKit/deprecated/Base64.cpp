...
std::string Base64::base64Encode( const std::string& str_in )
{
	std::string str;
...
std::string Base64::base64Decode( const std::string& str_in )
{
	std::string str;
...
void Base64::encode(FILE *fileHander, std::string& output, bool add_crlf)
{
	size_t remain;
	size_t i = 0;
	size_t o = 0;
	...
	output = "";
	remain = fread(input,1,3,fileHander);
	while (remain > 0)
	{
...
void Base64::encode(const std::string& input, std::string& output, bool add_crlf)
{
	encode(input.c_str(), input.size(), output, add_crlf);
}


void Base64::encode(const char* input,size_t len,std::string& output, bool add_crlf)
{
	size_t i = 0;
	size_t o = 0;
...
void Base64::encode(const unsigned char* input,size_t len,std::string& output,bool add_crlf)
{
	size_t i = 0;
	size_t o = 0;
