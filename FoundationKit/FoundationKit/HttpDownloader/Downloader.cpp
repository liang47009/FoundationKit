...
void Downloader::prepareDownload(const DownloadUnit& downloadUnit)
{
    ...
    if (_supportResuming && downloadUnit.resumeDownload && FileUtils::getInstance()->isFileExist(outFileName))
    {
        localFP = fopen(outFileName.c_str(), "ab");
    }
    else
    {
        localFP = fopen(outFileName.c_str(), "wb");
    }
    if (!localFP)
    {
...
{
    LOG_ASSERT(buffer, "must not be nill");

    LOG_ASSERT(_downloaderImpl, "Cannot instanciate more than one instance of DownloaderImpl");
...
void Downloader::downloadToFP(const std::string& srcUrl, const std::string& customId, const std::string& storagePath)
{
    ...
    if (res != 0)
    {
        ...
        FileUtils::getInstance()->removeFile(unit.storagePath + TEMP_EXT);
    ...
    if (res == 0)
    {
        FileUtils::getInstance()->renameFile(unit.storagePath + TEMP_EXT, unit.storagePath);
...
void Downloader::batchDownloadSync(const DownloadUnits& units, const std::string& batchId/* = ""*/)
{
    ...
    Scheduler::getInstance()->performFunctionInMainThread([ptr, batchId]
    {
        if (!ptr.expired()) {
            std::shared_ptr<Downloader> downloader = ptr.lock();
...
{
    LOG_ASSERT(userdata, "Invalid userdata");
    DownloadUnit* unit = (DownloadUnit*)userdata;
    FILE *fp = (FILE*)(unit->fp);
    
    LOG_ASSERT(fp, "Invalid FP");
...
size_t Downloader::bufferWriteFunc(void *ptr, size_t size, size_t nmemb, void *userdata)
{
    ...
    LOG_ASSERT(streamBuffer, "Invalid streamBuffer");
