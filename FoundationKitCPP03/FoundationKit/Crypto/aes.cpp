...
int aesSetupDecrypt(u32 *rk, const u8 *key, int keybits)
{
  ...
  for (int i = 0, j = 4*nrounds; i < j; i += 4, j -= 4)
  {
    tmpSwap = rk[i    ]; rk[i    ] = rk[j    ]; rk[j    ] = tmpSwap;
    tmpSwap = rk[i + 1]; rk[i + 1] = rk[j + 1]; rk[j + 1] = tmpSwap;
    tmpSwap = rk[i + 2]; rk[i + 2] = rk[j + 2]; rk[j + 2] = tmpSwap;
    tmpSwap = rk[i + 3]; rk[i + 3] = rk[j + 3]; rk[j + 3] = tmpSwap;
  }
